...
from BVP_Dictionary import Exact, P2,P1,P0,Q,a,b,ia,ib,alpha,beta,Num_Node_int,Num_Node_bca, Num_Node_bcb
...
test_log = []
loss_ch=torch.tensor([0.0,0.0,0.0],dtype=Dtype,requires_grad=True)
N_epoch_EBC=50 #delete
N_epoch_Plain = 20 #delete
for epoc in range(N_epoch_EBC):
    los = optimizer.step(Loss)
    if epoc%10==0:#N_write==0:
        print('\rEpoch : %d\tLoss: %3.2e\tL0: %3.2e\tL1: %3.2e\tL2: %3.2e'%(epoc,los,loss_ch[0],loss_ch[1],loss_ch[2]))
...
torch.save(Nn, "Output/Trained_Model.pt")

loss_arr = np.array(loss_log)
fig = plt.figure(figsize=(8,6),dpi=500)
ax = fig.add_subplot(111)
ax.semilogy(loss_arr[:,0],loss_arr[:,1],label='Predicted',color='tab:blue', linewidth = 2)
ax.set_xlabel('Epochs')
ax.set_ylabel('Loss')
#ax.axis('square')
#ax.set_xlim([0,0.5])
ax.set_title("Variational Form - Residual", fontsize = 15)
ax.legend()
...
#plt.savefig("Output/Variational Residual.pdf",bbox_inches='tight', pad_inches=0.5)
#plt.savefig("Output/Variational Dirichlet.eps",bbox_inches='tight', pad_inches=0.5)
plt.show()
...
ax.plot(x_arr.clone().detach().numpy(),u_exac.clone().detach().numpy(), 'r--', linewidth = 2,label='Exact')
#ax.plot(x_np,u_exac-u_pred, label = 'Difference',color='tab:red', linewidth = 2)
ax.set_xlabel('$x$')
ax.set_ylabel('$u$')
#ax.axis('square')
ax.set_xlim([a,b])
ax.set_title("Variational Form - Solution Comparison", fontsize = 15)
ax.legend()
...
#plt.savefig("Output/Variational Dirichlet.pdf",bbox_inches='tight', pad_inches=0.5)
#plt.savefig("Output/Variational Dirichlet.eps",bbox_inches='tight', pad_inches=0.5)
plt.show()
########################################################################################
#                                       Body
